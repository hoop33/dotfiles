(function() {
  module.exports = function() {
    return {
      ...
      getColorsInString: function(string) {
        var View, regex, type, _colors, _fn, _i, _j, _len, _len1, _match, _matches, _ref, _ref1;
        View = this;
        _colors = [];
        _ref = require('./modules/ColorRegexes');
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          _ref1 = _ref[_i], type = _ref1.type, regex = _ref1.regex;
          if (!(_matches = string.match(regex))) {
            continue;
          }
          _fn = (function(_this) {
            return function(type, _match) {
              var _index, _matchColor;
              ...
              _matchColor = {
                ...
                getSmartColor: function() {
                  switch (type) {
                    case 'rgb':
                      return View.SmartColor.RGB(_match);
                    case 'rgba':
                      return View.SmartColor.RGBA(_match);
                    case 'hsl':
                      return View.SmartColor.HSL(_match);
                    case 'hsla':
                      return View.SmartColor.HSLA(_match);
                    case 'hex':
                      return View.SmartColor.HEX(_match);
                    case 'hexa':
                      return View.SmartColor.HEXA(_match);
                    case 'vec3':
                      return View.SmartColor.VEC(_match);
                    case 'vec4':
                      return View.SmartColor.VECA(_match);
                    case 'hsv':
                      return View.SmartColor.HSV(_match);
                    case 'hsva':
                      return View.SmartColor.HSVA(_match);
                  }
                }
              };
      ...
      open: function() {
        var Cursor, Editor, EditorShadowRoot, EditorView, getDefinition, _colorMatches, _colorPickerPosition, _convertedColor, _cursorColumn, _cursorPosition, _cursorRow, _editorHeight, _editorOffsetLeft, _editorOffsetTop, _editorScrollTop, _editorWidth, _lineContent, _lineHeight, _lineOffsetLeft, _match, _position, _preferredFormat, _randomColor, _redColor, _ref, _selection, _selectionPosition, _totalOffsetLeft, _visibleRowRange;
        if (!this.canOpen) {
          return;
        }
        this.emitBeforeOpen();
        ...
        _colorMatches = this.getColorsInString(_lineContent);
        _cursorColumn = Cursor.getBufferColumn();
        _match = (function() {
          var _i, _len;
          for (_i = 0, _len = _colorMatches.length; _i < _len; _i++) {
            _match = _colorMatches[_i];
        ...
        if (_match) {
          if ((_ref = _match.type) === 'variable:sass' || _ref === 'variable:less') {
            this.emitInputVariable(_match);
            getDefinition = (function(_this) {
              return function(variable, type, pointer) {
                return (_this.Inspector(variable, type)).then(function(definition) {
                  var _colorMatch, _ref1;
                  if (!definition) {
                    throw new Error('No definition');
                  }
                  _colorMatch = (_this.getColorsInString(definition.definition))[0];
                  if (!_colorMatch) {
                    throw new Error('Definition not a color');
                  }
                  if (pointer == null) {
                    pointer = definition.pointer;
                  }
                  if ((_ref1 = _colorMatch.type) === 'variable:sass' || _ref1 === 'variable:less') {
            ...
            getDefinition(_match.match, _match.type).then((function(_this) {
              return function(color) {
                return _this.emitInputVariableColor(color.getSmartColor(), color.pointer);
              };
          ...
          _randomColor = this.SmartColor.RGB('rgb(' + ([((Math.random() * 255) + .5) << 0, ((Math.random() * 255) + .5) << 0, ((Math.random() * 255) + .5) << 0].join(', ')) + ')');
          _preferredFormat = atom.config.get('color-picker.preferredFormat');
          if (_randomColor.type !== _preferredFormat) {
          ...
          _redColor = this.SmartColor.RGB('rgb(255, 0, 0)');
          _preferredFormat = atom.config.get('color-picker.preferredFormat');
          if (_redColor.type !== _preferredFormat) {
